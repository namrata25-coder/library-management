...
static string NormalizePostgreSqlConnectionString(string value)
{
            ...
            || databaseUri.Scheme.Equals("postgresql", StringComparison.OrdinalIgnoreCase)))
    {
        var uriBuilder = new NpgsqlConnectionStringBuilder
        {
            ...
            Username = Uri.UnescapeDataString(databaseUri.UserInfo.Split(':')[0])
        };

        var passwordSeparator = databaseUri.UserInfo.IndexOf(':');
        if (passwordSeparator >= 0)
        {
            uriBuilder.Password = Uri.UnescapeDataString(
                databaseUri.UserInfo[(passwordSeparator + 1)..]);
        }
    ...
    if (connectionBuilder.Host.StartsWith("tcp://", StringComparison.OrdinalIgnoreCase))
    {
        var hostUri = new Uri(connectionBuilder.Host);
